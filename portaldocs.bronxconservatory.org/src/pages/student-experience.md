...
1. **Notes from Your Last Class** — the first thing under the greeting: the
   ...
   top right of the page. Both open one continuous chronological chat of
   every note and material the student has, each class marked only by a slim
   date separator the way messaging apps mark days. What's visible first is
   the most relevant thing — notes on the upcoming lesson closest to today —
   with earlier history collapsed behind a "View notes from previous
   lessons" button that expands it above, in order.
