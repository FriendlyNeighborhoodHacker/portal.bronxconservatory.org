<?php
// Add a student: the essentials only; everything else lives on Edit Student.
...
<div class="card">
  <form method="post" action="/admin/student_add_eval.php" class="stack">
    ...
    <div class="grid-2">
      ...
      <label>Email <span class="small">(optional — many students have none)</span>
        <input type="email" name="email" value="<?=h($old['email'] ?? '')?>"></label>
      <label>Class of <span class="small">(graduation year, optional)</span>
        <input type="number" name="class_of" min="2020" max="2050" value="<?=h($old['class_of'] ?? '')?>"></label>
    </div>
    <div>Instruments:</div>
    ...
    <div class="actions">
      ...
      <a class="button" href="/admin/students.php">Cancel</a>
    </div>
  </form>
</div>
