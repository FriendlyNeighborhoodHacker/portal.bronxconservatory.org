...
function send_smtp_mail(string $toEmail, string $toName, string $subject, string $html): bool {
  ...
  $fromName  = defined('SMTP_FROM_NAME') ? SMTP_FROM_NAME : 'BCM Family Portal';
...
function send_email_with_error(string $toEmail, string $subject, string $html, string $toName = '', string &$errorMessage = '', ?array $smtp = null, string $replyTo = ''): bool {
  ...
  if ($smtp !== null) {
    ...
    $fromName  = ($smtp['from_name'] ?? '') !== '' ? (string)$smtp['from_name'] : 'BCM Family Portal';
  } else {
    if (!defined('SMTP_HOST') || !defined('SMTP_PORT') || !defined('SMTP_USER') || !defined('SMTP_PASS')) {
...
/**
 * Convenience wrapper with logging. Returns true/false. Pass $smtp (an
 * override array) to send through a different SMTP server instead of the
 * global config, and/or $replyTo to set a Reply-To header — the two are
 * independent.
 */
...
function send_lead_registration_email(array $lead, array $students, array $quoteLines): bool {
  ...
  foreach ($students as $student) {
    ...
    if (trim((string)($student['shirt_size'] ?? '')) !== '') {
      $detail .= ', shirt size ' . $e($student['shirt_size']);
    }
